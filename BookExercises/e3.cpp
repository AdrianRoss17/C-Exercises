...
int main() {
	...
	cout << "Please enter homework grades for " << students.size() << " students (3 per): ";
	...
	{
		for (int j = studentsHomeworkSections; j < ; j++)
		{
			double avg = j
				homeWorkStudentsVectorAverage.push_back
		}
	}

	cout << "Please enter their final grade: ";
	...
	while (cin >> final)
	{
		double finalGrade = (0.2 * final) + (hw1 + hw2 + hw3) / 3;

		homeWorkVector.push_back(finalGrade);
	}
	...
	for (int i = 0; i < homeWorkVector.size(); ++i)
	{
		cout << homeWorkVector[i];
	}
}